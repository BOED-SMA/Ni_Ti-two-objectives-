...
% 3) DEFINE nE and nI
nE=20; % Preselected number of evaluations of he BOED process. Each evaluation correspods to an experiment
...
while m<=nI 
    ...
    [OUTPUT_VARIABLES, ERROR] = SYSTEMS_MODEL(CASE_NUMBER,NI_COMPO,VF);
    [OBJECTIVES_VALUE]=CALCULATE_OBJECTIVES(objectfun,OUTPUT_VARIABLES,ERROR);
...
function [OBJECTIVES_VALUE]=CALCULATE_OBJECTIVES(objectfun,OUTPUT_VARIABLES,ERROR)
...
while i<=nE  && i <=nRT    % IT WILL STOP WHEN THE STOPNUM IS REACHED OR WHEN THE ENTIRE SPACE IS COVERED. THE ENTIRE SPACE IS DEFINED AS THE INITIAL_SPACE-THE INITIAL RUNS-NOT ACCEPTABLE POINTS
    ...
    [INPUT p_integral]= SELECTOR(X,Y, Y_MEAN, Y_STD,XSPACE, qnum, dnum, boundpoint); % Select the next material to test (INPUT)
    NI_COMPO=INPUT(1,1);
    VF=INPUT(1,2);
    [idx, CASE_NUMBER]=IDENTIFY_IDX_AND_CASE_NUMBER(NI_COMPO,VF,XSPACE);
    [OUTPUT_VARIABLES, ERROR] = SYSTEMS_MODEL(CASE_NUMBER,NI_COMPO,VF);
...
function [INPUT p_integral]= SELECTOR(X,Y,Y_MEAN, Y_STD,XSPACE, qnum, dnum, boundpoint)
XSPACER=XSPACE(:,2:3);
left_idx = qnum+1:size(XSPACER, 1)-dnum;% from current number to the end of xspace
X_TEST=XSPACER(left_idx, :);
m=size(left_idx,2);
...
function [OUTPUT_VARIABLES, ERROR] = SYSTEMS_MODEL(CASE_NUMBER,NI_COMPO,VF)
